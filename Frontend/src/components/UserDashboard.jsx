...
function UploadModal({ mode, onClose, onSuccess }) {
  ...
  const handleFile = (f) => {
    if (!f) return;
    if (!f.type.startsWith("image/")) { setUploadError("Only image files accepted (JPG, PNG, WEBP)."); return; }
    setFile(f); setPreview(URL.createObjectURL(f)); setUploadError(null); setResult(null);
  };
            ...
            <div className="flex flex-col items-center">
              <div className="w-full aspect-[4/5] bg-black rounded-2xl overflow-hidden relative shadow-inner">
                ...
                <div className="absolute inset-0 pointer-events-none flex items-center justify-center">
                  <div className="w-48 h-48 border-2 border-white/50 rounded-2xl relative">
                    <div className="w-6 h-6 border-t-4 border-l-4 border-emerald-400 absolute top-0 left-0 rounded-tl-xl"/>
                    <div className="w-6 h-6 border-t-4 border-r-4 border-emerald-400 absolute top-0 right-0 rounded-tr-xl"/>
                    <div className="w-6 h-6 border-b-4 border-l-4 border-emerald-400 absolute bottom-0 left-0 rounded-bl-xl"/>
                    <div className="w-6 h-6 border-b-4 border-r-4 border-emerald-400 absolute bottom-0 right-0 rounded-br-xl"/>
                  </div>
                </div>
              </div>
              ...
              <div className="w-full bg-sky-50 border border-sky-100 rounded-xl p-3 mt-4 mb-5">
                ...
                <ul className="text-xs text-sky-700 space-y-1.5 font-medium">
                  ...
                  <li className="flex gap-2"><span>3.</span>Hold 4–6 inches away.</li>
                </ul>
              </div>
              {isStreaming && (
                <button onClick={capturePhoto} className="w-16 h-16 rounded-full bg-white border-4 border-slate-200 shadow-xl flex items-center justify-center hover:bg-slate-100 active:scale-95 transition-all ring-4 ring-sky-500/20">
            ...
            <>
              <div
                ...
                className={`relative border-2 border-dashed rounded-2xl text-center overflow-hidden transition-colors mb-4 ${
                  preview ? "border-transparent bg-slate-50 aspect-square" : "border-sky-300 p-8 cursor-pointer hover:bg-sky-50"
                }`}
              >
                {preview ? (
                  <>
                  ...
                  <div className="py-4">
                    ...
                    <p className="text-xs text-slate-500 font-medium leading-relaxed max-w-[200px] mx-auto">Drag & drop or click to browse files.</p>
                  </div>
                )}
              </div>
          ...
          {result && (
            <div className="flex flex-col items-center gap-5">
              ...
              <p className="text-xs text-slate-400">
                Raw score: <span className="font-mono font-semibold text-slate-600">{Number(result.raw_score ?? 0).toFixed(6)}</span>
              </p>
              ...
              <button onClick={onClose} className="w-full py-3.5 bg-slate-900 text-white rounded-xl font-black text-base hover:bg-slate-800 active:scale-[0.98] transition-all shadow-lg">
                Done
              </button>
            </div>
          )}
...
function SmartThumbnail({ url, prediction, confidence, createdAt, scanId }) {
  ...
  if (!url || hasError) {
    ...
    const confNum = isNaN(parseFloat(String(confidence || "0").replace("%",""))) ? "0.0" : parseFloat(String(confidence).replace("%","")).toFixed(1);
  ...
  if (error) {
    return (
      <div className="rounded-2xl border border-rose-200 bg-rose-50 px-5 py-8 text-center">
        ...
        {onRetry && <button onClick={onRetry} className="text-xs font-bold text-rose-600 border border-rose-300 px-4 py-2 rounded-lg hover:bg-rose-100 transition-colors">↻ Try Again</button>}
      </div>
    );
  }
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
      {history.map((scan) => (
        ...
        >
          <div className={`relative w-full h-full transition-transform duration-700 ease-in-out [transform-style:preserve-3d] shadow-lg rounded-3xl ${flippedCardId === scan.id ? "[transform:rotateY(180deg)]" : ""}`}>
            ...
            <div className={`absolute inset-0 w-full h-full [backface-visibility:hidden] rounded-3xl p-6 text-white overflow-hidden flex flex-col justify-between ${
              ...
              {scan.image_url && (
                <img src={scan.image_url} alt="Skin Scan" className="absolute inset-0 w-full h-full object-cover mix-blend-overlay opacity-10 pointer-events-none group-hover:opacity-30 transition-opacity duration-700"/>
              )}
            </div>
...
export default function UserDashboard({ user, onLogout }) {
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden">
      ...
      <aside className="hidden md:flex flex-col w-60 lg:w-64 bg-white border-r border-slate-200 shadow-sm shrink-0">
          ...
          {NAV_ITEMS.map((item) => (
            <button key={item.id} onClick={() => setActiveTab(item.id)}
              className={`w-full flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm font-semibold transition-all ${
                activeTab === item.id ? "bg-sky-600 text-white shadow-md shadow-sky-200" : "text-slate-500 hover:bg-slate-100 hover:text-slate-800"
              }`}
            >
        ...
        <div className="px-4 pb-4">
          <div className="bg-gradient-to-br from-sky-600 to-sky-700 rounded-xl p-4 text-white">
            ...
            <button className="w-full bg-white text-sky-700 text-xs font-bold py-2 rounded-lg hover:bg-sky-50 transition-colors">Get Support</button>
          </div>
        </div>
        <div className="px-4 pb-5 border-t border-slate-100 pt-3">
          <button onClick={handleLogout} className="w-full flex items-center gap-2 px-3.5 py-2.5 rounded-xl text-sm font-semibold text-slate-500 hover:bg-rose-50 hover:text-rose-600 transition-colors">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <nav className="md:hidden fixed bottom-0 inset-x-0 bg-white border-t border-slate-200 shadow-lg z-30 safe-bottom">
          <div className="flex">
            {NAV_ITEMS.map((item) => (
              <button key={item.id} onClick={() => setActiveTab(item.id)}
                className={`flex-1 flex flex-col items-center gap-0.5 py-2.5 transition-colors ${activeTab === item.id ? "text-sky-600" : "text-slate-400"}`}
              >
                <div className={`p-1 rounded-lg transition-all ${activeTab === item.id ? "bg-sky-100" : ""}`}>{item.icon}</div>
      ...
      {uploadModal && <UploadModal mode={uploadModal} onClose={() => setUploadModal(null)} onSuccess={handleScanSuccess}/>}
    </div>
  );
}
    ...
    <div className="px-4 sm:px-6 lg:px-8 py-6 max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        <button onClick={onCamera} className="flex flex-col items-start gap-3 bg-sky-600 text-white p-4 sm:p-5 rounded-2xl shadow-lg shadow-sky-200 hover:bg-sky-700 active:scale-[0.98] transition-all text-left">
          ...
          <div><div className="font-black text-sm">Open Camera</div><div className="text-xs text-sky-200 mt-0.5">Capture live photo</div></div>
        </button>
        <button onClick={onScan} className="flex flex-col items-start gap-3 bg-white border border-slate-200 p-4 sm:p-5 rounded-2xl shadow-sm hover:shadow-md hover:border-sky-300 active:scale-[0.98] transition-all text-left">
          ...
          <div><div className="font-black text-sm text-slate-800">Upload File</div><div className="text-xs text-slate-400 mt-0.5">From your gallery</div></div>
        </button>
      </div>
      <div className="mb-6">
    ...
    <div className="px-4 sm:px-6 py-6 max-w-xl mx-auto">
      ...
      <div className="space-y-4">
        <button onClick={onCamera} className="w-full flex items-center gap-4 p-5 bg-sky-600 text-white rounded-2xl shadow-lg shadow-sky-200 hover:bg-sky-700 transition-all active:scale-[0.99]">
          ...
          <div className="text-left flex-1"><div className="font-black">Open Camera</div><div className="text-sm text-sky-200">Live capture with your device camera</div></div>
        ...
        <button onClick={onUpload} className="w-full flex items-center gap-4 p-5 bg-white border border-slate-200 rounded-2xl shadow-sm hover:shadow-md hover:border-sky-300 transition-all active:scale-[0.99]">
          ...
          <div className="text-left flex-1"><div className="font-black text-slate-800">Upload from Gallery</div><div className="text-sm text-slate-400">JPG, PNG, WEBP — up to 10 MB</div></div>
      ...
      <div className="mt-8 bg-amber-50 border border-amber-200 rounded-xl p-4">
        <p className="text-xs text-amber-800 leading-relaxed"><span className="font-bold">📸 Photo Tips:</span> Good lighting, steady hand, centre the area of concern. Avoid shadows for best accuracy.</p>
      </div>
    </div>
  );
}
    ...
    <div className="px-4 sm:px-6 py-6 max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-slate-500 text-sm">{loading ? "Loading…" : `${history.length} total scan${history.length !== 1 ? "s" : ""}`}</p>
        </div>
        {!loading && <button onClick={onRetry} className="text-xs text-sky-600 font-semibold border border-sky-200 px-3 py-1.5 rounded-lg hover:bg-sky-50 transition-colors">↻ Refresh</button>}
      </div>
...
function DoctorTab() {
  ...
  const handleSearch = () => { if (searchInput.trim()) setMapQuery(`dermatologist in ${searchInput}`); };
  ...
  return (
    <div className="px-4 sm:px-6 py-8 max-w-3xl mx-auto">
      ...
      <div className="flex gap-3 mb-6">
        ...
        <button onClick={handleSearch} className="px-6 py-3 bg-slate-900 text-white rounded-2xl text-sm font-black hover:bg-slate-800 transition-colors shadow-lg active:scale-95 shrink-0">Search</button>
      </div>
      ...
      <div className="bg-sky-50 border border-sky-100 rounded-2xl p-4 flex gap-3 items-center">
        ...
        <p className="text-xs text-sky-800 font-medium">Click the red pins in the map to see clinic names, ratings, and phone numbers from Google.</p>
      </div>
    </div>
  );
}
    ...
    <div className="px-4 sm:px-6 py-8 max-w-2xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 flex items-center gap-4 hover:shadow-md transition-shadow">
          ...
          <div><div className="text-2xl font-black text-slate-800">{scanCount}</div><div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Total Scans</div></div>
        </div>
        <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 flex items-center gap-4 hover:shadow-md transition-shadow">
          ...
          <div><div className="text-xl font-black text-slate-800">v2.4.1</div><div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">App Version</div></div>
        </div>
      </div>
